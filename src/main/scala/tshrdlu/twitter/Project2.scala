...
//.replaceAll("""\b@[A-Za-z]+[0-9]*[A-Za-z]*\b""", "Justin Bieber")
    if(tshrdlu.util.English.isEnglish(text) && tshrdlu.util.English.isSafe(text) && !text.contains('/') && !text.contains('@') && text.contains('#'))
      println(text)
  }
